...
from math import atan, pi, sqrt, cos, fabs, sin
...
cajas = "15 17 10 15 15 10"

# Agente maestro?
iamMaster = True

nombreAgente = "robot2@127.0.0.1"
# Si hubiera mas de un agente esto seria una lista
otroAgente = "robot1@127.0.0.1"
...
def goForBoxes(controller, msg, counter):
    ...
    if counter % 2 == 0:
        ...
        while fabs(objX-realPos.x) > 0.01 or fabs(objY-realPos.y) > 0.01 :
            ...
            realPos = almath.Pose2D(controller.locationProxy.getRobotPosition(False))
        """
        controller.motionProxy.post.moveTo(x, y, alpha)
        controller.motionProxy.waitUntilMoveIsFinished()
        realPos = almath.Pose2D(controller.locationProxy.getRobotPosition(False))
